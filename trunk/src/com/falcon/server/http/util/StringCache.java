...
public class StringCache {
	...
	private static Logger log = Logger.getLogger(StringCache.class);
				...
				synchronized (bcStats) {
					...
					if (bcCount > trainThreshold) {
						...
						while (n < size) {
							...
							ByteEntry[] list2 = (ByteEntry[]) list.toArray(new ByteEntry[list
									.size()]);
				...
				synchronized (ccStats) {
					...
					if (ccCount > trainThreshold) {
						...
						while (n < size) {
							Object key = tempMap.lastKey();
							ArrayList list = (ArrayList) tempMap.get(key);
							CharEntry[] list2 = (CharEntry[]) list.toArray(new CharEntry[list
									.size()]);
							for (int i = 0; i < list.size() && n < size; i++) {
